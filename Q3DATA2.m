...
fprintf(sprintf("svm error for data2 is: %2.f precent\n",test_err_svm));
...
plot(xx,line1,'--',xx,line2,'--');hold off;title("Svm for data2 with Margin");
%% perceptron
subplot(122)
[theta_Per] = perceptron_train_2020(X,y,false);
test_err_per=my_peceptron_test(theta_Per,1,X,y);
fprintf(sprintf("perceptron error for data2 is: %2.f precent\n",test_err_per));
...
plot(xx,line1,'--');
plot(xx,line2,'--');
hold off;title("Perceptron for data2 with margin");
%% B
%% SVM
r1=gammaGeom(X,theta_svm);
figure(2);subplot(121);plot_points_and_boundary(X,y,theta_svm);hold on;
plot(xx,y1);plot(xx,y1+r1,'--');plot(xx,y1-r1,'--');hold off;;title("Svm for data2 with gamma geometry");
%% perceptron
...
title("Perceptron for data2 with gamma geometry");
